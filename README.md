...
Huidige versie: **v2.29** (planner als dagkaarten). De inplan-weergave (`meal-scheduler.js`) is van een dicht weekrooster omgebouwd naar rustige dagkaarten in agenda-stijl: per dag een kaart met de slots eronder (icoon, label, gerecht), strakke monochrome SVG-iconen in plaats van emoji, lege slots vervaagd met "toevoegen". Klik een regel om het gerecht daar te plannen; vandaag en het bij het type passende slot worden gemarkeerd. Op mobiel één kolom, op breder scherm meerdere kaarten naast elkaar.
